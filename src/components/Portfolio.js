				...
				{selectedProject && (
					<Lightbox
						...
						prevSrc={
							this.state.images.length !== 1 ? (
								this.state.images[
									(photoIndex + this.state.images.length - 1) % this.state.images.length
								]
							) : null
						}
						onCloseRequest={() => {
						...
						onMovePrevRequest={() =>
							this.setState({
								[key]: (photoIndex + this.state.images.length - 1) % this.state.images.length
							})}
						onMoveNextRequest={() =>
							this.setState({
						...
						enableZoom={false}
						imageCaption={
							photoIndex ? selectedProjctData.name[lang] + ' feature' : selectedProjctData.name[lang]
						}
					/>
				)}
				<Fade>
					<div name="portfolio" id="portfolio" className="wrapper portfolio page">
						<h2>Portfolio</h2>
						<section className="projects" onClick={this.handleClick}>
									...
									return (
										<React.Fragment key={data.name[lang]}>
											<div className="project-container">
												<figure
													id={data.order}
													className={`project-img${!data.gallery.length
														? ' no-gallery'
														: ''}`}
												>
							...
							<button className="ui grey basic button" onClick={this.handleNumberOfProject}>
